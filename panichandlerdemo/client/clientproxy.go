...
func (receiver *TestsvcClientProxy) GetGreeting(ctx context.Context, _headers map[string]string) (_resp *resty.Response, re error) {
	if _err := receiver.runner.Run(ctx, func(ctx context.Context) error {
		_resp, re = receiver.client.GetGreeting(
			ctx,
			_headers,
		)
		if re != nil {
			return errors.Wrap(re, "call GetGreeting fail")
		}
		return nil
	}); _err != nil {
		...
		re = errors.Wrap(_err, "call GetGreeting fail")
	}
	return
}
