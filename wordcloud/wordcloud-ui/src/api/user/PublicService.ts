...
import type { PublicLogInResp, PublicSignUpResp, PublicTokenValidateResp } from './types';
...
username
  ...
  */
  postPublicLogIn(params: { password: string; username: string }): Promise<PublicLogInResp> {
...
username
  ...
  */
   ...
   */
  postPublicTokenValidate(params: { token: string }): Promise<PublicTokenValidateResp> {
    let client = this.axios.post;
    if (this.axios.$post) {
      client = this.axios.$post;
    }
    ...
    return client(this.addPrefix(`/public/token/validate`), urlSearchParams, {
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
      },
    });
