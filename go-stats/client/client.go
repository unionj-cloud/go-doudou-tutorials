...
func (receiver *GoStatsClient) GetBook(ctx context.Context, _headers map[string]string, price decimal.Decimal, options Options) (_resp *resty.Response, data string, err error) {
	var _err error
	_urlValues := url.Values{}
	_req := receiver.client.R()
	if len(_headers) > 0 {
	...
	_req.SetContext(ctx)
	_urlValues.Set("price", fmt.Sprintf("%v", price))
	_path := "/book"
	_req.SetQueryParamsFromValues(_urlValues)
	_resp, _err = _req.Get(_path)
	if _err != nil {
