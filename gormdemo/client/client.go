...
func (receiver *GormdemoClient) PageUsers(ctx context.Context, _headers map[string]string, query dto.PageQuery, options Options) (_resp *resty.Response, data dto.PageRet, err error) {
	var _err error
	...
	if options.GzipReqBody {
		...
		go func() {
			...
			_err = json.NewEncoder(gw).Encode(query)
			if _err != nil {
